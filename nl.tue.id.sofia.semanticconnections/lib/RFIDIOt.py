...
#	Added acs_halt() command - 09/11/2009
#	Removed self.reset() from acs_select_tag() - 09/11/2009
...
class rfidiot:
	...
	# open reader port
	#
	def __init__(self,readernum,reader,port,baud,to,debug,noinit):
		global NoInit
		global DEBUG
				...
				if string.find(self.readername,'OMNIKEY') == 0:
					...
					if string.find(self.readername,'SDI010') == 0:
						...
						if string.find(self.readername,'ACS ACR 38U') == 0:
                            ...
                            # default to ACS with protocol T0
							self.readersubtype= self.READER_ACS
							self.pcsc_protocol= smartcard.scard.SCARD_PROTOCOL_T0
							self.hcard = None
	...
	# PCSC_Keys= { key : (keytype,keynum) } where keynum is 0 - 31 as per OmniKey docs
	PCSC_Keys= {}
	# Static Globals
	...
	def acs_transmit_apdu(self,apdu):
		...
		if self.hcard is None:
			...
			# and we ignore failure of we're running on formware V1 as it doesn't support this command
	...
	def acs_get_firmware_revision(self):
		...
		self.errorcode= self.ISO_NOINFO
		return False
	#Added by Gerrit 10/11/2009
	def acs_halt(self):
		"ACS Halt"
		result = self.acs_send_apdu(self.PCSC_APDU['ACS_HALT'])
		return result
...
#		return ret[4:int(le,16) * 2 + 4]
	...
	def login(self,sector,keytype,key):
		...
		if self.readertype == self.READER_PCSC:
			if self.readersubtype == self.READER_ACS:
				if self.acs_mifare_login(sector,key):
					return True
				else:
					return False
			...
			if not self.PCSC_Keys.has_key(key):
				...
				apdu.append('%02x' % keynum) # p2 - key number
				...
				if self.readersubtype == self.READER_OMNIKEY:
					...
					self.PCSC_Keys[key]= (keytype,keynum)
			else:
				#use stored key if already sent
				keytype, keynum= self.PCSC_Keys[key]
