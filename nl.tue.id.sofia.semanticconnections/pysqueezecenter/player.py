...
class Player(object):
    ...
    def playlist_load_tracks_title(self, title):
        """Searches for track title and loads into playlist"""
        return self.request('playlist loadtracks track.titlesearch=' + self.__quote(title))
    ...
    def set_alarm_add(self, dow, time, repeat=0, volume=None, url=""):
        """Add new alarm"""
        alarmCommand = "alarm add dow:"+str(dow)+" time:"+time+" enabled:1 repeat:"+str(repeat);
        
        if(url != ""):
            alarmCommand += " url:"+url;
        
        if( (volume!=None) and (0 <= int(volume) <= 100) ):
            alarmCommand += " volume:"+str(volume);    
        
        print alarmCommand      
        alarm_id = self.request(alarmCommand)
        return alarm_id[5:] # squeezebox returns "k id:" before actual id
        
    def get_alarm_playlists(self, number):
        print self.request("alarm playlists 0 %i" % int(number))
        
    def get_alarms(self, number, param="enabled"):
        """ Returns info about player alarms. param:enabled(default)/all """
        return self.request("alarms 0 %i filter:%s" % (int(number), param))
        
    def delete_alarm(self, id):
        """Delete an existing alarm"""
        self.request("alarm delete id:" +str(id))
    
    def stop_alarm(self):
        """Stop Alarm"""
        self.request("irenable 1")
        self.request("button stop")
        #self.request("button back")
        #self.request("button done_passback")
        #self.request("button done")
        #self.request("button snooze")
        #self.request("jivealarm stop:1")
        #self.request("pause 1")
