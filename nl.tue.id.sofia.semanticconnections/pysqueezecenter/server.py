...
class Server(object):
    ...
    def subscribe(self, msg_handler, player_id):
        self.conn = telnetlib.Telnet(self.hostname, self.port)
        self.conn.write(self.__encode("listen 1" + "\n"))
        sub_h = SubscribeHandler(self.conn,msg_handler, player_id)
        sub_h.start()
...
"""
To handle the subscribe functionality in your code, you need something like this:
...
and call 
    ...
    sc.subscribe(SBMsgHandler(),player_id)

in your main code after sc.connect(), where `player_id` is a string.

"""            
            
class SubscribeHandler(threading.Thread):

    def __init__(self, connector, msg_handler, player_id):
    ...
    def run(self):
        while True:
            ...
            if(tokens[0].strip() == self.player_id.lower()):
                msgArray = tokens[1:]
                self.msg_handler.handle(msgArray)
            
    def read(self):
        response = self.__decode(self.conn.read_until(self.__encode("\n"))[:-1])
        response = self.__unquote(response)
