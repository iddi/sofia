...
class Query(Transaction):
    ...
    def _create_sparql_msg(self, tr_id, query_string):
        tmp = self._create_msg_header(tr_id)
        tmp.extend(['<parameter name = "type">sparql</parameter>'])
        tmp.extend(['<parameter name = "query">'])
        tmp.extend([query_string])
        tmp.extend(['</parameter></SSAP_message>'])
        return "".join(tmp)
    ...
    def sparql_query(self, string_q):
        query_string=self.esamina_stringa(string_q)
        self.tr_id = str(uuid.uuid1())
        sparql_msg = self._create_sparql_msg(self.tr_id, query_string)
        self.conn.connect()
        self.conn.send(sparql_msg)
        response = self.conn.receive()
        if "results" in response:
           triple_list=parse_sparql(response["results"])
           return triple_list
        else:
           return [] 
    
    def esamina_stringa(self,stringa_orig):
